node {
    ...
    stage('Build Project') {
        dir("/var/lib/jenkins/workspace/initial-deploy/kubernetes") {
            sh "aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin aaaaaaaaaaaaa.dkr.ecr.us-east-1.amazonaws.com"
            sh "docker build -t python:built ."
            sh "docker tag python:built aaaaaaaaaaaaa.dkr.ecr.us-east-1.amazonaws.com/python:latest"
            sh "docker push aaaaaaaaaaaaa.dkr.ecr.us-east-1.amazonaws.com/python:latest"
    ...
    stage('login to aws registry') {
        dir("/var/lib/jenkins/workspace/initial-deploy/kubernetes") {
            sh "sshpass -p 'Operation123' ssh -o StrictHostKeyChecking=no root@xxx.xxx.xxx.xxx 'aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin aaaaaaaaaaaaa.dkr.ecr.us-east-1.amazonaws.com'"
            sh "sshpass -p 'Operation123' ssh -o StrictHostKeyChecking=no root@yyy.yyy.yyy.yyy 'aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin aaaaaaaaaaaaa.dkr.ecr.us-east-1.amazonaws.com'"
        }
    }
    ...
    stage('Build python') {
        dir("/var/lib/jenkins/workspace/initial-deploy/kubernetes") {
            ...
            sh "sshpass -p 'Operation123' ssh -o StrictHostKeyChecking=no root@xxx.xxx.xxx.xxx 'kubectl create secret docker-registry regcred --docker-server=aaaaaaaaaaaaa.dkr.ecr.us-east-1.amazonaws.com --docker-username=AWS --docker-password=\$(aws ecr get-login-password --region us-east-1)'"
